...
#[wasm_bindgen]
pub fn frost_encode_identifier(id: u16) -> Result<Vec<u8>, JsError> {
    let ident = Identifier::try_from(id).map_err(to_js_err)?;
    Ok(ident.serialize())
}

#[wasm_bindgen]
pub fn frost_decode_identifier(id_bytes: &[u8]) -> Result<u16, JsError> {
...
mod tests {
    ...
    fn test_encode_decode_identifier() {
        for id in 1..=10u16 {
            let encoded = frost_encode_identifier(id).unwrap();
            let decoded = frost_decode_identifier(&encoded).unwrap();
