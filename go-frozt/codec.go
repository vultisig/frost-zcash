...
func DecodeMap(data []byte) ([]MapEntry, error) {
	if len(data) < 4 {
		return nil, fmt.Errorf("frozt: codec: data too short")
	}
	...
	for i := 0; i < count; i++ {
		if pos+4 > len(data) {
			return nil, fmt.Errorf("frozt: codec: truncated at key length %d", i)
		}
		klen := int(binary.LittleEndian.Uint32(data[pos : pos+4]))
		pos += 4
		if pos+klen > len(data) {
			return nil, fmt.Errorf("frozt: codec: truncated at key data %d", i)
		}
		key := make([]byte, klen)
		...
		if pos+vlen > len(data) {
			return nil, fmt.Errorf("frozt: codec: truncated at value data %d", i)
		}
		val := make([]byte, vlen)
