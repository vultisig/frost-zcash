...
func RunSign(ctx context.Context, client *RelayClient, sessionID, partyID string, identifier uint16, keyPackage, pubKeyPackage, message []byte, signerParties []string) (*SignResult, error) {
	...
	nonces, commitmentBytes, err := frost.SignCommit(keyPackage)
	if err != nil {
	...
	commitmentsEncoded := frost.EncodeMap(commitmentsMap)
...
func runCoordinator(ctx context.Context, client *RelayClient, sessionID, partyID string, identifier uint16, nonces frost.Handle, keyPackage, pubKeyPackage, message, commitmentsEncoded []byte, signerParties []string) (*SignResult, error) {
	signingPackage, randomizer, err := frost.SignNewPackage(message, commitmentsEncoded, pubKeyPackage)
	if err != nil {
	...
	myShare, err := frost.Sign(signingPackage, nonces, keyPackage, randomizer)
	if err != nil {
	...
	myIDBytes, err := frost.EncodeIdentifier(identifier)
	if err != nil {
		return nil, fmt.Errorf("encode my id: %w", err)
	}

	allShareEntries := []frost.MapEntry{{ID: myIDBytes, Value: myShare}}
	for _, s := range shares {
		idBytes, err := frost.EncodeIdentifier(s.SenderID)
		if err != nil {
		...
		allShareEntries = append(allShareEntries, frost.MapEntry{ID: idBytes, Value: shareData})
	}

	sharesEncoded := frost.EncodeMap(allShareEntries)
	signature, err := frost.SignAggregate(signingPackage, sharesEncoded, pubKeyPackage, randomizer)
	if err != nil {
...
func runSigner(ctx context.Context, client *RelayClient, sessionID, partyID string, identifier uint16, nonces frost.Handle, keyPackage, message, commitmentsEncoded, pubKeyPackage []byte, signerParties []string) (*SignResult, error) {
	...
	share, err := frost.Sign(signingPackage, nonces, keyPackage, randomizer)
	if err != nil {
		return nil, fmt.Errorf("sign: %w", err)
	}
...
func buildCommitmentsMap(commitments []CommitmentMessage) []frost.MapEntry {
	entries := make([]frost.MapEntry, 0, len(commitments))
	for _, c := range commitments {
		idBytes, err := frost.EncodeIdentifier(c.SenderID)
		if err != nil {
			continue
		}
		...
		entries = append(entries, frost.MapEntry{ID: idBytes, Value: data})
	}
	return entries
}
