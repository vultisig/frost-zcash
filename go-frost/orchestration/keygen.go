...
func RunKeygen(ctx context.Context, client *RelayClient, sessionID, partyID string, identifier, maxSigners, minSigners uint16, allParties []string) (*KeygenResult, error) {
	secret1, round1Pkg, err := frost.DkgPart1(identifier, maxSigners, minSigners)
	if err != nil {
	...
	keyPackage, pubKeyPackage, err := frost.DkgPart3(secret2, round1Encoded, round2Encoded)
	if err != nil {
...
func buildRoundMap(messages []RoundMessage) []frost.MapEntry {
	entries := make([]frost.MapEntry, 0, len(messages))
	for _, m := range messages {
		idBytes, err := frost.EncodeIdentifier(m.SenderID)
		if err != nil {
			continue
		}
		...
		entries = append(entries, frost.MapEntry{
			ID:    idBytes,
			Value: data,
		})
	}
...
func sendPerRecipient(ctx context.Context, client *RelayClient, sessionID, partyID, messageID string, senderIdentifier uint16, mapEntries []frost.MapEntry, allParties []string) error {
	partyMap := buildPartyIdentifierMap(allParties)

	for _, entry := range mapEntries {
		recipientID, err := frost.DecodeIdentifier(entry.ID)
		if err != nil {
