...
#[no_mangle]
pub extern "C" fn frozt_derive_z_address(
...
#[no_mangle]
pub extern "C" fn frozt_derive_t_address(
...
#[no_mangle]
pub extern "C" fn frozt_pubkey_to_t_address(
    pub_key_package: Option<&go_slice>,
    out_address: Option<&mut tss_buffer>,
) -> lib_error {
    with_error_handler(|| {
...
mod tests {
    ...
    fn test_derive_z_address() {
        ...
        assert_eq!(
            frozt_derive_z_address(Some(&pkp_slice), Some(&mut addr)),
            lib_error::LIB_OK,
        );
    ...
    fn test_derive_t_address() {
        ...
        let mut addr = tss_buffer::empty();
        assert_eq!(
            frozt_pubkey_to_t_address(Some(&pkp_slice), Some(&mut addr)),
            lib_error::LIB_OK,
        );
